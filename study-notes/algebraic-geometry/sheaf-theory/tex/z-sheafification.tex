...
\,$\sigma, \,\tau \in \mathscr{F}(U)$\,
defined on a common (open) domain \,$U \subset X$\,
of a presheaf \,$\mathscr{F}$\, over a topological space \,$X$,\,
the agreement of \,$\sigma$\, and \,$\tau$\, in the stalk at each \,$x \in U$\,
is equivalent ONLY to the agreement of
\,$\sigma$\, and \,$\tau$\,
AFTER restrictions to the constituent open subsets of a certain open cover
\,$U \,=\, \underset{\alpha}{\bigcup}\;U_{\alpha}$.\,
In particular,
the agreement of \,$\sigma$\, and \,$\tau$\, in the stalk at each \,$x \in U$\,
does NOT imply that
\,$\sigma$\, and \,$\tau$\, are equal as sections in \,$\mathscr{F}(U)$.
\end{remark}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{remark}[Local set-theoretic description of $\mathscr{P}^{\flat}$ and $\mathscr{P}^{\sharp}$ of a presheaf $\mathscr{P}$]
\mbox{}\vskip 0.1cm
\noindent
...
& := &
	\left\{\;\,
		...
		\,\;\left\vert\;
			...
			\textnormal{there exists an open cover \,$U \,=\, \underset{\alpha}{\bigcup}\;U_{\alpha}$\, such that}
			\\
			\textnormal{$\left.\left(\overset{{\color{white}.}}{(\,\sigma_{x}\,)_{x \in U}}\right)\,\right\vert^{(\mathscr{D}(\mathscr{P}))}_{\,U_{\alpha}} \,\in\, \mathscr{P}(U_{\alpha})$\,,\, for each \,$\alpha$}
			\end{array}
			\right.
		\right\}
%\\ & {\color{white}\textnormal{\large$1$}} &
