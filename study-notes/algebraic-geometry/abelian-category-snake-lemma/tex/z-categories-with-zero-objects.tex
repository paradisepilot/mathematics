...
\item
	an \textbf{epimorphism} if it can be {\color{red}right} cancelled in the sense that
...
\vskip 0.5cm
\begin{proposition}
\label{KernelsAreMonoCokernelsAreEpi}
\mbox{}
\vskip 0.1cm
\noindent
...
\item
	We have already established -- see proof of (iii) -- that $\widetilde{f}$ is an epimorphism.
...
\item
	...
	such that \,$f \,=\, \kappa(\pi_{f}) \circ \varepsilon_{f}$.
\item
	The morphism \,$\varepsilon_{f}$\, is a coimage of \,$f$,\, and
	it is a cokernel of a kernel of \,$f$.
\item
	There exists a unique morphism \,$\theta \in \Mor_{\mathfrak{C}}(I_{f},K_{g})$\,
	such that \,$\kappa(\pi_{f}) \,=\, \kappa_{g} \circ \theta$.\,
	Furthermore, this morphism \,$\theta$\, is a monomorphism.
\end{enumerate}
\begin{center}
\begin{tikzcd}
&& && {\color{blue}Q_{f}}
	\arrow[dd, blue, thick, dashed, "\,\exists !\,\psi_{g}"]
\\ \\
A
	\arrow[rr, thick, "f"]
	\arrow[dd, red, thick, dashed, two heads, "\varepsilon_{f}", swap]
...
I_{f}
	...
	\arrow[rr, red, thick, hook, dashed, "\exists !\,\theta", swap]
&&
{\color{blue}K_{g}}
	\arrow[uu, blue, thick, hook, "\kappa_{g}", swap]
\end{tikzcd}
\end{center}
\end{proposition}
\proof
...
To this end, first note that, since
\,$g \,\circ\, f \,=\, 0$,\,
there exists a unique morphism
\,$\psi_{g} \in \Mor_{\mathfrak{C}}(Q_{f},C)$\,
such that
\,$g \,=\, \psi_{g} \,\circ\, \pi_{f}$,\,
by the universal property of \,$\pi_{f}$\, as a cokernel of \,$f$.\,
Next, note that, by the universal property of \,$\kappa_{g}$\, as a kernel of \,$g$,\,
the desired unique morphism \,$\theta$\, will exist provided that \,$g \circ \kappa(\pi_{f}) = 0$,\,
which in turn easily follows from:
\begin{equation*}
g \circ \kappa(\pi_{f})
\;\; = \;\;
	\left(\,\psi_{g} \overset{{\color{white}!}}{\circ} \pi_{f}\,\right) \circ \kappa(\pi_{f})
\;\; = \;\;
	\psi_{g} \,\circ \left(\,\pi_{f} \overset{{\color{white}!}}{\circ} \kappa(\pi_{f})\,\right)
\;\; = \;\;
	\psi_{g} \,\circ\, 0
\;\; = \;\;
	0
\end{equation*}
It remains only to show that \,$\theta$\, is a monomorphism.
To this end, simply observe:
\begin{eqnarray*}
\theta \circ h_{1} \,=\, \theta \circ h_{2}
& \Longrightarrow &
	\kappa_{g} \circ \theta \circ h_{1} \,=\, \kappa_{g} \circ \theta \circ h_{2}
\\
& \Longrightarrow &
	\kappa(\pi_{f}) \circ h_{1} \,=\, \kappa(\pi_{f}) \circ h_{2}\,,
	\quad
	\textnormal{since \,$\kappa(\pi_{f}) \,=\, \kappa_{g} \circ \theta$,\, by construction of $\theta$}
\\
& \Longrightarrow &
	h_{1} \,=\, h_{2}\,,
	\quad
	\textnormal{since kernels are monomorphisms, by Proposition \ref{KernelsAreMonoCokernelsAreEpi}}
\end{eqnarray*}
This proves that \,$\theta$\, is indeed a monomorphism and completes the proof of the Proposition.
\qed
...
K_{g}
	...
	\arrow[rr, red, thick, two heads, "\pi_{\theta}", swap]
&&
Q_{\theta}
\end{tikzcd}
\end{center}
...
\vskip 0.5cm
\begin{corollary}
\label{MorphismCokernelExactness}
\mbox{}
\vskip 0.1cm
\noindent
Suppose \,$\mathfrak{C}$\, is a category with zero objects,
\,$f \in \Mor_{\mathfrak{C}}(A,B)$\, is a morphism in \,$\mathfrak{C}$,\, and
\,$\pi_{f} \in \Mor_{\mathfrak{C}}(B,Q_{f})$\, is a cokernel of \,$f$.\,
...
\end{corollary}
\proof
Observe that replacing \,$g$\, in the diagram in Definition \ref{defnExactness} 
with the cokernel \,$\pi_{f}$\, of \,$f$\, yields the following:
\begin{center}
\begin{tikzcd}
&& && Q_{f}
	\arrow[dd, blue, thick, dashed, "\psi\,=\,1_{Q_{f}}"]
\\ \\
A
	\arrow[rr, thick, "f"]
	\arrow[dd, thick, dashed, two heads, swap, "\varepsilon_{f}"]
	\arrow[rrrr, gray, bend left = 30, "0"]
&&
B
	\arrow[rr, blue, thick, two heads, "\pi_{f}"]
	\arrow[uurr, thick, two heads, "\pi_{f}"]
&&
{\color{blue}Q_{f}}
\\ \\
I_{f}
	\arrow[uurr, thick, hook, "\kappa(\pi_{f})"]
	\arrow[rr, red, thick, dashed, hook, "\exists !\,\theta", swap]
&&
{\color{blue}I_{f}}
	\arrow[uu, blue, thick, hook, swap, "\kappa(\pi_{f})"]
	\arrow[rr, red, thick, two heads, "\pi_{\theta}", swap]
&&
Q_{\theta}
\end{tikzcd}
\end{center}
The uniqueness of \,$\theta$\, now implies that
\,$\theta$\, must in fact be the identity morphism of \,$I_{f}$.\,
In particular, \,$\theta$\, is then an isomorphism.
