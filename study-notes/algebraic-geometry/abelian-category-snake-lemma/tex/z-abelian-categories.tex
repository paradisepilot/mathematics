
          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\section{Abelian categories}
\setcounter{theorem}{0}
\setcounter{equation}{0}

%\cite{vanDerVaart1996}
%\cite{Kosorok2008}

%\renewcommand{\theenumi}{\alph{enumi}}
%\renewcommand{\labelenumi}{\textnormal{(\theenumi)}$\;\;$}
\renewcommand{\theenumi}{\roman{enumi}}
\renewcommand{\labelenumi}{\textnormal{(\theenumi)}$\;\;$}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\begin{definition}[Abelian category]
\mbox{}
\vskip 0.15cm
\noindent
An additive category \,$\mathfrak{A}$\, is said to be \textbf{abelian}
if it furthermore satisfies each of the following conditions:
\begin{itemize}
\item
	finite products exist in \,$\mathfrak{A}$\,,
\item
	every morphism in \,$\mathfrak{A}$\, has a kernel as well as a cokernel,
\item
	every monomorphism in \,$\mathfrak{A}$\, is a kernel, and
\item
	every epimorphism in \,$\mathfrak{A}$\, is a cokernel.
\end{itemize}
\end{definition}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

% [Proposition 1.2.9, p.24, \cite{Lal2021Algebra3}; Theorem 2.12, p.37, \cite{freyd1964abelian}]
\vskip 0.5cm
\begin{proposition}[Proposition 5.5, p.203, \cite{Grillet2007}]
\label{AbelianImpliesIsomorphismIFFMonoEpi}
\mbox{}
\vskip 0.1cm
\noindent
In each abelian category, a morphism is an isomorphism if and only if
it is both a monomorphism and an epimorphism.
\end{proposition}
\proof
\vskip 0.1cm
\noindent
\underline{($\Longrightarrow$)}
\vskip 0.2cm
\noindent
Suppose \,$f \in \Mor_{\mathfrak{A}}(A,B)$\, is an isomorphism; thus, there exists some
\,$f^{-1} \in \Mor_{\mathfrak{A}}(B,A)$\,
such that
\,$f \circ f^{-1} = 1_{B}$\, and \,$f^{-1} \circ f = 1_{A}$.\,
First, we prove that \,$f$\, is a monomorphism.
So, we suppose
\,$g, h \in \Mor_{\mathfrak{C}}(C,A)$\,
are such that
\,$f \circ g = f \circ h \in \Mor_{\mathfrak{A}}(C,B)$,\,
and we need to prove that \,$g = h \in \Mor_{\mathfrak{A}}(C,A)$.\,
To this end, simply observe:
\begin{equation*}
g
\; = \;
	1_{A} \circ g
\; = \;
	(f^{-1} \circ f) \circ g
\; = \;
	f^{-1} \circ (f \circ g)
\; = \;
	f^{-1} \circ (f \circ h)
\; = \;
	(f^{-1} \circ f) \circ h
\; = \;
	1_{A} \circ h
\; = \;
	h
\end{equation*}
This proves that \,$f$\, is indeed a monomorphism.
Next, we establish that \,$f$\, is an epimorphism.
So, we suppose
\,$g, h \in \Mor_{\mathfrak{A}}(B,C)$\,
are such that
\,$g \circ f = h \circ f \in \Mor_{\mathfrak{A}}(A,C)$,\,
and we need to prove
\,$g = h$.\,
To this end, simply observe:
\begin{equation*}
g
\; = \;
	g \circ 1_{B}
\; = \;
	g \circ (f \circ f^{-1})
\; = \;
	(g \circ f) \circ f^{-1}
\; = \;
	(h \circ f) \circ f^{-1}
\; = \;
	h \circ (f \circ f^{-1})
\; = \;
	h \circ 1_{B}
\; = \;
	h
\end{equation*}
This completes the proof that \,$f$\, is indeed both a monomorphism and an epimorphism.

\vskip 0.3cm
\noindent
\underline{($\Longleftarrow$)}
\vskip 0.2cm
\noindent
Conversely, suppose \,$f \in \Mor_{\mathfrak{A}}(A,B)$\, is both a monomorphism and an epimorphism.
Since \,$\mathfrak{A}$\, is an abelian category, the morphism \,$f$\, admits a cokernel
\,$\pi \in \Mor_{\mathfrak{A}}(B,Q)$.\,

\vskip 0.3cm
\noindent
\textbf{Claim 1:}\quad $\pi \,=\, 0_{B,Q}$
\vskip 0.1cm
\noindent
Proof of Claim 1:\;\;
$\pi$\, being a cokernel of \,$f$\, implies
\,$\pi \circ f = 0_{A,Q} = 0_{B,Q} \circ f$.\,
Since \,$f$\, is an epimorphism (i.e., ``right-cancellable''), it follows that
\,$\pi = 0_{B,Q}$.\,
This proves Claim 1.

\vskip 0.3cm
\noindent
\textbf{Claim 2:}\quad
There exists \,$\theta \,\in\, \Mor_{\mathfrak{A}}(B,A)$\, such that \,$f \circ \theta \,=\, 1_{B}$
\vskip 0.1cm
\noindent
Proof of Claim 2:\;\;
Since \,$\mathfrak{A}$\, is an abelian category,
the monomorphism \,$f$\, is a kernel of some morphism.
Hence, by Lemma \ref{AKernelsTheKernelOfItsOwnCokernel} (see also Lemma 5.4, p.203, \cite{Grillet2007}),
the monomorphism \,$f$\, is a kernel of its own cokernel \,$\pi$.\,
Now, by Claim 1, we have:
\,$\pi \,\circ\, 1_{B} \,=\, 0_{B,Q} \,\circ\, 1_{B} \,=\, 0_{B,Q}$,\,
which in turn implies
-- by the universal property of $f$ as kernel of $\pi$ --
that there exists a unique 
\,$\theta \in \Mor_{\mathfrak{A}}(B,A)$\,
such that
\,$1_{B} \,=\, f \circ \theta$.\,
See the following diagram for the universality of \,$f$\, as a kernel of \,$\pi$:
% Then αβα = α = α 1A and βα = 1A.
\begin{center}
\begin{tikzcd}
{\color{blue}B}
	\arrow[dd, thick, dashed, swap, "\exists !\,\theta{\color{white}.}", blue]
	\arrow[dr, thick, swap, "1_{B}\!\!", blue]
	\arrow[drrr, bend left = 15, "0", blue] & & &
\\
& {\color{red}B}
	\arrow[rr, swap, "\pi\,=\,0_{B,Q}{\color{white}...}"] & & Q
\\
{\color{red}A}
	\arrow[ur, swap, "f", red]
	\arrow[urrr, bend right = 15, swap, "0", gray]
\end{tikzcd}
\end{center}
This proves Claim 2.

\vskip 0.3cm
\noindent
\textbf{Claim 3:}\quad
$\theta \circ f \,=\, 1_{A}$
\vskip 0.1cm
\noindent
Proof of Claim 3:\;\;
Note that
\,$f \,\circ\, (\theta \,\circ\, f) \,=\, (f \,\circ\, \theta) \,\circ\, f \,=\, 1_{B} \,\circ\, f \,=\, f \,=\, f \,\circ\, 1_A$,\,
which implies
\,$\theta \circ f \,=\, 1_{A}$,\,
since
\,$f$\, is a monomorphism (i.e., ``left-cancellable'').
This proves Claim 3.

\vskip 0.3cm
\noindent
The Proposition now follows from Claim 2 and Claim 3.
\qed

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{proposition}\label{MonomorphicFirstFactorInExactSequenceIsKernelOfSecondFactor}
\mbox{}
\vskip 0.1cm
\noindent
Suppose \,$\mathfrak{A}$\, is an abelian category,
\,$f \in \Mor_{\mathfrak{A}}(A,B)$, and $g \in \Mor_{\mathfrak{A}}(B,C)$\,
such that
\begin{itemize}
\item
	$f$\, is a monomorphism,
\item
	$g \circ f = 0$,\, and
\item
	$A \overset{f}{\longrightarrow} B \overset{g}{\longrightarrow} C$\,
	is exact.
\end{itemize}
Then, \,$f$\, is a kernel of \,$g$.
\end{proposition}
\proof
By Proposition \ref{propositionHomology} or Definition \ref{defnExactness},
the exactness of
\,$A \overset{f}{\longrightarrow} B \overset{g}{\longrightarrow} C$\,
means precisely that the unique morphism \,$\theta$\, in the following diagram is an isomorphism:
\begin{center}
\begin{tikzcd}
&& && Q_{f}
	\arrow[dd, dashed, "\psi"]
\\ \\
A
	\arrow[rr, "f"]
	\arrow[dd, dashed, two heads, swap, "\varepsilon_{f}"]
	\arrow[rrrr, bend left = 30, "0", gray]
&&
B
	\arrow[rr, "g"]
	\arrow[uurr, two heads, "\pi_{f}"]
&&
C
\\ \\
I_{f}
	\arrow[uurr, hook, "\kappa(\pi_{f})"]
	\arrow[rr, thick, dashed, swap, "\exists !\,\theta", red]
&&
K_{g}
	\arrow[uu, hook, swap, "\kappa_{g}"]
	%\arrow[rr, thick, two heads, swap, "\pi_{\theta}", red]
&&
%Q_{\theta}
\end{tikzcd}
\end{center}
It is clear, by the above commutative diagram, that the Propossition will follow
as soon as we establish that \,$\varepsilon_{f}$\, is an isomorphism
(since \,$\theta$\, is an isomorphism by the exactness hypothesis).
To this end, it suffices to show that \,$\varepsilon_{f}$\, is both a monomormphism
as well as an epimorphism.
\vskip 0.3cm
\noindent
\textbf{Claim 1:}\quad $\varepsilon_{f}$\, is an epimorphism
\vskip 0.1cm
\noindent
Proof of Claim 1:\;\;
By Proposition \ref{propositionHomology}, \,$\varepsilon_{f}$\, is a cokernel of a kernel of \,$f$.\,
By Proposition \ref{KernelsAreMonoCokernelsAreEpi}, \,$\varepsilon_{f}$\, is an epimorphism.
This proves Claim 1.

\vskip 0.3cm
\noindent
\textbf{Claim 2:}\quad $\varepsilon_{f}$\, is an monomorphism
\vskip 0.1cm
\noindent
Proof of Claim 2:\;\;
Since \,$\mathfrak{A}$\, is an abelian category, it in particular has cokernels and is normal.
By Proposition \ref{FactorizationIntoImageCoimage}, we see that
\,$\kappa(\pi_{f})$\, is an image of \,$f$.\,
Since \,$\mathfrak{A}$\, is an abelian category, it in particular is normal and has cokernels and equalizers.
By Proposition \ref{FactorizationIntoImageCoimage} again, we see that
\,$\varepsilon_{f}$\, is a coimage of \,$f$, which we denote by \,$\widetilde{f}$\, from now on.
By Lemma \ref{fTildeIsMonomorphismWheneverFIs},
since \,$f$\, is a monomorphism, it follows that \,$\varepsilon_{f} \,=\, \widetilde{f}$\,
is itself a monomorphism.
This proves Claim2 2.

\vskip 0.3cm
\noindent
Thus, 
\,$\varepsilon_{f} \,=\, \widetilde{f}$\,
is both a monomorphism and an epimorphism.
By Proposition \ref{AbelianImpliesIsomorphismIFFMonoEpi}, we see that
\,$\varepsilon_{f} \,=\, \widetilde{f}$\,
is an isomorphism.
Thus,
\,$f \,=\, \kappa_{g} \,\circ\, \theta \,\circ\, \varepsilon_{f}$,\,
where \,$\theta$\, and \,$\varepsilon_{f}$\, are isomorphisms.
Since \,$\kappa_{g}$\, is a kernel of \,$g$,\,
we may now conclude that \,$f$\, is also a kernel of \,$g$.\, 
This completes the proof of the Proposition.
\qed

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{proposition}[p.176, \cite{kashiwara2005categories}]
\label{FiberProductFiberCoproductExistInAbelianCategories}
\mbox{}
\vskip 0.1cm
\noindent
In each abelian category,
fiber products and fiber coproducts always exist.
\end{proposition}
\proof
\begin{enumerate}
\item
	We first prove the existence of fiber products in an arbitrary abelian category.
	Suppose \,$\mathfrak{A}$\, is an abelian category,
	\,$f_{1} \in \Mor_{\mathfrak{A}}(X_{1},Y)$\,
	and
	\,$f_{2} \in \Mor_{\mathfrak{A}}(X_{1},Y)$.\,
	Since \,$\mathfrak{A}$\, is an abelian category, the finite {\color{red}biproduct}
	\begin{equation*}
	\left(\,
		X_{1} \overset{{\color{white}.}}{\oplus} X_{2}
		\,,\,
		p_{1}
		\,,\,
		p_{2}
		\,,\,
		\iota_{1}
		\,,\,
		\iota_{2}
		\,\right)
	\end{equation*}
	of \,$X_{1},\, X_{2}$\, exists, where
	\,$p_{1} : X_{1} \oplus X_{2} \longrightarrow X_{1}$,\,
	\,$p_{2} : X_{1} \oplus X_{2} \longrightarrow X_{2}$,\,
	\,$\iota_{1} : X_{1} \longrightarrow X_{1} \oplus X_{2}$,\,
	\,$\iota_{2} : X_{2} \longrightarrow X_{1} \oplus X_{2}$\,
	satisfy:
	\begin{equation*}
	p_{1} \circ \iota_{1} = 1_{X_{1}}\,,
	\quad
	p_{2} \circ \iota_{2} = 1_{X_{2}}\,,
	\quad
	p_{1} \circ \iota_{2} = 0_{X_{2},X_{1}}\,,
	\quad
	p_{2} \circ \iota_{1} = 0_{X_{1},X_{2}}
	\end{equation*}
	\vskip 0.3cm
	\noindent
	\textbf{Claim 1:}\quad
	Let \,$\kappa \,\in\, \Mor_{\mathfrak{A}}(K, X_{1}\oplus X_{2})$\,
	be any {\color{red}kernel of
	\,$\varphi\,:=\,f_{1}\,\circ\,p_{1}\,-\,f_{2}\,\circ\,p_{2}\,\in\,\Mor_{\mathfrak{A}}(\,X_{1} \oplus X_{2}\,,\,Y\,)$}.\,
	Define
	\,$\pi_{1} := p_{1} \circ \kappa \in \Mor_{\mathfrak{A}}(K,X_{1})$\,
	and
	\,$\pi_{2} := p_{2} \circ \kappa \in \Mor_{\mathfrak{A}}(K,X_{2})$.\,
	Then,
	\,$\left(\,K\,,\,\pi_{1}\,,\,\pi_{2}\,\right)$\,
	is a fiber product of
	\,$f_{1}$\,
	and
	\,$f_{2}$.\,
	\vskip 0.2cm
	\noindent
	Proof of Claim 1:\;\;
	First, observe that:
	\begin{eqnarray*}
	f_{1} \circ \pi_{1} - f_{2} \circ \pi_{2}
	& = &
		f_{1} \circ (\,p_{1} \circ \kappa\,) - f_{2} \circ (\,p_{2} \circ \kappa\,)
	\;\; = \;\;
		(\,f_{1} \circ p_{1}\,) \circ \kappa - (\,f_{2} \circ p_{2}\,) \circ \kappa
	\\
	& = &
		(\,f_{1} \circ p_{1} - f_{2} \circ p_{2}\,) \circ \kappa
	\;\; = \;\;
		\varphi \circ \kappa
	\\
	& = &
		0
	\end{eqnarray*}
	Next, let
	\,$h_{1} \in \Mor_{\mathfrak{A}}(A,X_{1})$\,
	and
	\,$h_{2} \in \Mor_{\mathfrak{A}}(A,X_{2})$\,
	be such that
	\,$f_{1} \circ h_{1} \,=\, f_{2} \circ h_{2}$.\,
	Define
	{\color{red}
	\begin{equation*}
	\psi
	\;\; := \;\;
		\iota_{1}\circ h_{1}\,+\,\iota_{2}\circ h_{2}
	\;\; \in \;\;
		\Mor_{\mathfrak{A}}(\,A\,,\,X_{1} \oplus X_{2}\,)
	\end{equation*}
	}
	Observe:
	\begin{eqnarray*}
	\varphi \circ \psi
	& = &
		\left(\,f_{1} \circ\, p_{1} \,\overset{{\color{white}.}}{-}\, f_{2} \circ\, p_{2}\,\right)
		\,\circ\,
		\left(\,\iota_{1} \circ h_{1} \,\overset{{\color{white}.}}{+}\, \iota_{2} \circ h_{2}\,\right)
	\\
	& = &
		f_{1} \,\circ\, p_{1} \,\circ\, \iota_{1} \,\circ\, h_{1}
		\; + \;
		f_{1} \,\circ\, p_{1} \,\circ\, \iota_{2} \,\circ\, h_{2}
		\; - \;
		f_{2} \,\circ\, p_{2} \,\circ\, \iota_{1} \,\circ\, h_{1}
		\; - \;
		f_{2} \,\circ\, p_{2} \,\circ\, \iota_{2} \,\circ\, h_{2}
	\\
	& \overset{{\color{white}1}}{=} &
		f_{1} \,\circ\, 1_{X_{1}} \,\circ\, h_{1}
		\; + \;
		f_{1} \circ\, 0_{X_{2},X_{1}} \,\circ\, h_{2}
		\; - \;
		f_{2} \circ\, 0_{X_{1},X_{2}} \,\circ\, h_{1}
		\; - \;
		f_{2} \,\circ\, 1_{X_{2}} \,\circ\, h_{2}
	\\
	& \overset{{\color{white}1}}{=} &
		f_{1} \,\circ\, h_{1}
		\; - \;
		f_{2} \,\circ\, h_{2}
	\;\; = \;\;
		0
	\end{eqnarray*}
	Thus, by the universal property of
	\,$\kappa \,\in\, \Mor_{\mathfrak{A}}(K,X_{1} \oplus X_{2})$\,
	as kernel of
	\,$\varphi\,:=\,f_{1}\,\circ\,p_{1}\,-\,f_{2}\,\circ\,p_{2}\,\in\,\Mor_{\mathfrak{A}}(X_{1} \oplus X_{2},Y)$,\,
	there exists a unique
	\,$\theta \in \Mor_{\mathfrak{A}}(A,K)$\,
	such that
	\,$\psi = \kappa \circ \theta$.\,
	Furthermore, \,$\theta$\, satisfies the following two equalities:
	%\vskip 0.1cm
	\begin{eqnarray*}
	\pi_{1} \circ \theta
	& = &
		(\,p_{1} \circ \kappa\,) \circ \theta
	\;\; = \;\;
		p_{1} \circ (\,\kappa \circ \theta\,)
	\;\; = \;\;
		p_{1} \circ \psi
	\;\; = \;\;
		p_{1} \circ (\,\iota_{1}\circ h_{1}\,+\,\iota_{2}\circ h_{2}\,)
	\\
	& = &
		(\,p_{1} \circ \iota_{1}\,) \circ h_{1}
		\,+\
		(\,p_{1} \circ \iota_{2}\,) \circ h_{2}
	\;\; = \;\;
		1_{X_{1}} \circ h_{1}
		\,+\
		0_{X_{2},X_{1}} \circ h_{2}
	\\
	& = &
		h_{1}
	\\ \\
	\pi_{2} \circ \theta
	& = &
		(\,p_{2} \circ \kappa\,) \circ \theta
	\;\; = \;\;
		p_{2} \circ (\,\kappa \circ \theta\,)
	\;\; = \;\;
		p_{2} \circ \psi
	\;\; = \;\;
		p_{2} \circ (\,\iota_{1}\circ h_{1}\,+\,\iota_{2}\circ h_{2}\,)
	\\
	& = &
		(\,p_{2} \circ \iota_{1}\,) \circ h_{1}
		\,+\
		(\,p_{2} \circ \iota_{2}\,) \circ h_{2}
	\;\; = \;\;
		0_{X_{1},X_{2}} \circ h_{1}
		\,+\,
		1_{X_{2}} \circ h_{2}
	\\
	& = &
		h_{2}
	\end{eqnarray*}
	% It remains to establish the uniqueness of \,$\theta$.\,
	% To this end, suppose
	% \,$\theta^{\prime} \in \Mor_{\mathfrak{A}}(A,K)$\,
	% is any morphism that satisfies
	% \begin{equation*}
	% \psi \,=\, \kappa \circ \theta^{\prime}\,,
	% \quad
	% \pi_{1} \circ \theta^{\prime} \,=\, h_{1}\,,
	% \quad
	% \pi_{2} \circ \theta^{\prime} \,=\, h_{2}\,,
	% \end{equation*}
	The above arguments are summarized in the following commutative diagram:
	\begin{center}
	\begin{tikzcd}
	&&&& {\color{blue}X_{1}}
		\arrow[ddrr, "f_{1}", blue]
		\arrow[ddl, bend left = 10, "\iota_{1}", hook, gray]
	\\ \\
	A
		\arrow[rr, dashed, "\;\;\;\exists\,!\,\theta"]
		\arrow[uurrrr, bend left =  20, "h_{1}"]
		\arrow[ddrrrr, bend left = -20, "h_{2}", swap]
		%\arrow[rrr, bend left = -20, "\psi\,:=\,\iota_{1}\circ h_{1} + \iota_{2}\circ h_{2}", swap]
		\arrow[rrr, bend left = -20, "\psi", swap]
	&&
	%{\color{red}X_{1} \sqcap_{Y}\! X_{2}}
	{\color{red}K}
		\arrow[r, "\kappa", hook, gray]
		\arrow[uurr, "{\color{red}\pi_{1}}", bend left =  10, red]
		\arrow[ddrr, "{\color{red}\pi_{2}}", bend left = -10, red, swap]
	&
	{\color{gray}X_{1} \oplus X_{2}}
		\arrow[rrr, "\varphi\,:=\,f_{1} \circ\, p_{1} \,-\, f_{2} \circ\, p_{2}", gray]
		\arrow[uur, "p_{1}", bend left =  10, gray]
		\arrow[ddr, "p_{2}", bend left = -10, gray, swap]
	&&&
	{\color{blue}Y}
	\\ \\
	&&&& {\color{blue}X_{2}}
		\arrow[uurr, "f_{2}", swap, blue]
		\arrow[uul, bend left = -10, "\iota_{2}", hook, gray, swap]
	\end{tikzcd}
	\end{center}
	This completes the proof of Claim 1,
	as well as that of the existence of fiber products in an arbitrary abelian category.
\item
	We now prove the existence of fiber coproducts in an arbitrary abelian category.
	Suppose \,$\mathfrak{A}$\, is an abelian category,
	\,$f_{1} \in \Mor_{\mathfrak{A}}(Y,X_{1})$\,
	and
	\,$f_{2} \in \Mor_{\mathfrak{A}}(Y,X_{1})$.\,
	Since \,$\mathfrak{A}$\, is an abelian category, the finite {\color{red}biproduct}
	\begin{equation*}
	\left(\,
		X_{1} \overset{{\color{white}.}}{\oplus} X_{2}
		\,,\,
		p_{1}
		\,,\,
		p_{2}
		\,,\,
		\iota_{1}
		\,,\,
		\iota_{2}
		\,\right)
	\end{equation*}
	of \,$X_{1},\, X_{2}$\, exists, where
	\,$p_{1} : X_{1} \oplus X_{2} \longrightarrow X_{1}$,\,
	\,$p_{2} : X_{1} \oplus X_{2} \longrightarrow X_{2}$,\,
	\,$\iota_{1} : X_{1} \longrightarrow X_{1} \oplus X_{2}$,\,
	\,$\iota_{2} : X_{2} \longrightarrow X_{1} \oplus X_{2}$\,
	satisfy:
	\begin{equation*}
	p_{1} \circ \iota_{1} = 1_{X_{1}}\,,
	\quad
	p_{2} \circ \iota_{2} = 1_{X_{2}}\,,
	\quad
	p_{1} \circ \iota_{2} = 0_{X_{2},X_{1}}\,,
	\quad
	p_{2} \circ \iota_{1} = 0_{X_{1},X_{2}}
	\end{equation*}
	\vskip 0.3cm
	\noindent
	\textbf{Claim 2:}\quad
	Let \,$\pi \,\in\, \Mor_{\mathfrak{A}}(\,X_{1}\oplus X_{2}\,,\,Q\,)$\,
	be any {\color{red}cokernel of
	\,$\varphi\,:=\,\iota_{1}\,\circ\,f_{1}\,-\,\iota_{2}\,\circ\,f_{2}\,\in\,\Mor_{\mathfrak{A}}(\,Y\,,\,X_{1} \oplus X_{2}\,)$}.\,
	Define
	\,$\alpha_{1} := \pi \circ \iota_{1} \in \Mor_{\mathfrak{A}}(X_{1},Q)$\,
	and
	\,$\alpha_{2} := \pi \circ \iota_{2} \in \Mor_{\mathfrak{A}}(X_{2},Q)$.\,
	Then,
	\,$\left(\,Q\,,\,\alpha_{1}\,,\,\alpha_{2}\,\right)$\,
	is a fiber coproduct of
	\,$f_{1}$\,
	and
	\,$f_{2}$.\,
	\vskip 0.2cm
	\noindent
	Proof of Claim 2:\;\;
	First, observe that:
	\begin{eqnarray*}
	\alpha_{1} \circ f_{1} - \alpha_{2} \circ f_{2}
	& = &
		(\,\pi \circ \iota_{1}\,) \circ f_{1} - (\,\pi \circ \iota_{2}\,) \circ f_{2}
	\;\; = \;\;
		\pi \circ (\,\iota_{1} \circ f_{1}\,) - \pi \circ (\,\iota_{2} \circ f_{2}\,)
	\\
	& = &
		\pi \circ (\,\iota_{1} \circ f_{1} - \iota_{2} \circ f_{2}\,) 
	\;\; = \;\;
		\pi \circ \varphi
	\\
	& = &
		0
	\end{eqnarray*}
	Next, let
	\,$h_{1} \in \Mor_{\mathfrak{A}}(X_{1},A)$\,
	and
	\,$h_{2} \in \Mor_{\mathfrak{A}}(X_{2},A)$\,
	be such that
	\,$h_{1}\,\circ\,f_{1} \,=\, h_{2}\,\circ\,f_{2}$.\,
	Define
	{\color{red}
	\begin{equation*}
	\psi
	\;\; := \;\;
		h_{1}\,\circ\,p_{1}\,+\,h_{2}\,\circ \,p_{2}
	\;\; \in \;\;
		\Mor_{\mathfrak{A}}(\,X_{1}\oplus X_{2}\,,A\,)
	\end{equation*}
	}
	Observe:
	\begin{eqnarray*}
	\psi \circ \varphi
	& = &
		\left(\,h_{1} \circ\, p_{1} \,\overset{{\color{white}.}}{+}\, h_{2} \circ\, p_{2}\,\right)
		\,\circ\,
		\left(\,\iota_{1} \circ f_{1} \,\overset{{\color{white}.}}{-}\, \iota_{2} \circ f_{2}\,\right)
	\\
	& = &
		h_{1} \,\circ\, p_{1} \,\circ\, \iota_{1} \,\circ\, f_{1}
		\; + \;
		h_{1} \,\circ\, p_{1} \,\circ\, \iota_{2} \,\circ\, f_{2}
		\; - \;
		h_{2} \,\circ\, p_{2} \,\circ\, \iota_{1} \,\circ\, f_{1}
		\; - \;
		h_{2} \,\circ\, p_{2} \,\circ\, \iota_{2} \,\circ\, f_{2}
	\\
	& \overset{{\color{white}1}}{=} &
		h_{1} \,\circ\, 1_{X_{1}} \,\circ\, f_{1}
		\; + \;
		h_{1} \circ\, 0_{X_{2},X_{1}} \,\circ\, f_{2}
		\; - \;
		h_{2} \circ\, 0_{X_{1},X_{2}} \,\circ\, f_{1}
		\; - \;
		h_{2} \,\circ\, 1_{X_{2}} \,\circ\, f_{2}
	\\
	& \overset{{\color{white}1}}{=} &
		h_{1} \,\circ\, f_{1}
		\; - \;
		h_{2} \,\circ\, f_{2}
	\;\; = \;\;
		0
	\end{eqnarray*}
	Thus, by the universal property of
	\,$\pi \,\in\, \Mor_{\mathfrak{A}}(\,X_{1} \oplus X_{2}\,,\,Q\,)$\,
	as cokernel of
	\,$\varphi\,:=\,\iota_{1}\,\circ\,f_{1}\,-\,\iota_{2}\,\circ\,f_{2}\,\in\,\Mor_{\mathfrak{A}}(\,Y\,,\,X_{1} \oplus X_{2}\,)$,\,
	there exists a unique
	\,$\theta \in \Mor_{\mathfrak{A}}(Q,A)$\,
	such that
	\,$\psi = \theta \circ \pi$.\,
	Furthermore, \,$\theta$\, satisfies the following two equalities:
	%\vskip 0.1cm
	\begin{eqnarray*}
	\theta \circ \alpha_{1}
	& = &
		\theta \circ (\,\pi \circ \iota_{1}\,)
	\;\; = \;\;
		(\,\theta \circ \pi\,) \circ \iota_{1}
	\;\; = \;\;
		\psi \circ \iota_{1} 
	\;\; = \;\;
		(\,h_{1} \circ p_{1} \,+\, h_{2} \circ p_{2} \,) \circ \iota_{1}
	\\
	& = &
		h_{1} \circ (\,p_{1} \circ \iota_{1}\,)
		\,+\
		h_{2} \circ (\,p_{2} \circ \iota_{1}\,)
	\;\; = \;\;
		h_{1} \circ 1_{X_{1}}
		\,+\
		h_{2} \circ 0_{X_{1},X_{2}}
	\\
	& = &
		h_{1}
	\\ \\
	\theta \circ \alpha_{2}
	& = &
		\theta \circ (\,\pi \circ \iota_{2}\,)
	\;\; = \;\;
		(\,\theta \circ \pi\,) \circ \iota_{2}
	\;\; = \;\;
		\psi \circ \iota_{2}
	\;\; = \;\;
		(\,h_{1} \circ p_{1} \,+\, h_{2} \circ p_{2} \,) \circ \iota_{2}
	\\
	& = &
		h_{1} \circ (\,p_{1} \circ \iota_{2}\,)
		\,+\
		h_{2} \circ (\,p_{2} \circ \iota_{2}\,)
	\;\; = \;\;
		h_{1} \circ 0_{X_{2},X_{1}}
		\,+\
		h_{2} \circ 1_{X_{2}}
	\\
	& = &
		h_{2}
	\end{eqnarray*}
	% It remains to establish the uniqueness of \,$\theta$.\,
	% To this end, suppose
	% \,$\theta^{\prime} \in \Mor_{\mathfrak{A}}(A,K)$\,
	% is any morphism that satisfies
	% \begin{equation*}
	% \psi \,=\, \kappa \circ \theta^{\prime}\,,
	% \quad
	% \pi_{1} \circ \theta^{\prime} \,=\, h_{1}\,,
	% \quad
	% \pi_{2} \circ \theta^{\prime} \,=\, h_{2}\,,
	% \end{equation*}
	The above arguments are summarized in the following commutative diagram:
	\begin{center}
	\begin{tikzcd}
	&&&& {\color{blue}X_{1}}
		\arrow[ddl, bend left = 10, "\iota_{1}", hook, gray]
		\arrow[ddllll, bend left = -20, "h_{1}", swap]
		\arrow[ddll, "{\color{red}\alpha_{1}}", bend left = -10, red, swap]
	\\ \\
	A
	&&
	%{\color{red}X_{1} \sqcup_{Y}\! X_{2}}
	{\color{red}Q}
		%\arrow[r, "\kappa", hook, gray]
		\arrow[ll, dashed, "\;\;\;\exists\,!\,\theta", swap]
	&
	{\color{gray}X_{1} \oplus X_{2}}
		\arrow[l, "\quad\pi", gray, swap]
		\arrow[uur, "p_{1}", bend left =  10, gray]
		\arrow[ddr, "p_{2}", bend left = -10, gray, swap]
		\arrow[lll, bend left = 20, "\psi"]
	&&&
	{\color{blue}Y}
		\arrow[uull, "f_{1}", blue, swap]
		\arrow[ddll, "f_{2}", blue]
		\arrow[lll, "\varphi\,:=\, \iota_{1} \circ\, f_{1} \,-\, \iota_{2} \circ\, f_{2}", gray, swap]
	\\ \\
	&&&& {\color{blue}X_{2}}
		\arrow[uull, "{\color{red}\alpha_{2}}", bend left = 10, red]
		\arrow[uullll, bend left = 20, "h_{2}"]
		\arrow[uul, bend left = -10, "\iota_{2}", hook, gray, swap]
	\end{tikzcd}
	\end{center}
	This completes the proof of Claim 2,
	as well as that of the existence of fiber coproducts in an arbitrary abelian category.
	\qed
\end{enumerate}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\begin{lemma}\label{AlternativeCharacterizationOfEpimorphismsInAbelianCategories}
\mbox{}
\vskip 0.1cm
\noindent
Let \,$\mathfrak{A}$\, be an {\color{red}abelian} category, and
\,$f \in \Mor_{\mathfrak{A}}(\,A,B\,)$,\, where
\,$A, B \in \Obj(\mathfrak{A})$.\,
Suppose \,$f$\, satisfies the following property:
\begin{equation*}
g \circ f = 0
\;\;\Longrightarrow\;\;
g = 0,
\quad
\textnormal{for each \,$g \in \Mor_{\mathfrak{A}}(B,C)$\, and for each\,$C \in \Obj(\mathfrak{A})$}.
\end{equation*}
Then, \,$f$\, is an epimorphism.
\end{lemma}
\proof
Let \,$C \in \Obj(\mathfrak{A})$\, be an arbibrary object in \,$\mathfrak{A}$\,
and \,$g_{1},\, g_{2} \in \Mor_{\mathfrak{A}}(B,C)$\, arbitrary morphisms satisfying
\,$g_{1} \circ f = g_{2} \circ f$.\, We need show \,$g_{1} = g_{2}$.\,
To this end, simply observe:
\begin{eqnarray*}
g_{1} \circ f \;=\; g_{2} \circ f
& \Longrightarrow &
	(\,g_{1} - g_{2}\,) \circ f \;=\; 0
\\
& \Longrightarrow &
	g_{1} - g_{2} \;=\; 0,
	\quad
	\textnormal{by hypothesis on \,$f$}
\\
& \Longrightarrow &
	g_{1} \;=\; g_{2}\,,
\end{eqnarray*}
as required.
\qed

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{proposition}[Fiber products preserve epimorphisms in {\color{red}abelian} categories; Lemma 2.4.3, p.45, \cite{Yekutieli2020}]
\label{FiberProductsPreserveEpimorphismsInAbelianCategories}
\mbox{}
\vskip -0.3cm
\noindent
Let \,$\mathfrak{A}$\, be an {\color{red}abelian} category.
\begin{enumerate}
\item
	\vskip -0.1cm
	Let
	\,$\left(\,X_{1}\overset{{\color{white}.}}{\sqcap_{Y}}\!X_{2} \,,\, \pi_{1} \,,\, \pi_{2}\,\right)$\,
	be a fiber product of two morphisms
	\,$f_{1} \in \Mor_{\mathfrak{A}}(X_{1},Y)$\,
	and
	\,$f_{2} \in \Mor_{\mathfrak{A}}(X_{2},Y)$,\,
	where
	\,$X_{1},\, X_{2},\, Y,\, X_{1}\sqcap_{Y}\!X_{2} \,\in\, \Obj(\mathfrak{C})$,\,
	\,$\pi_{1} \in \Mor_{\mathfrak{A}}(\,X_{1}\sqcap_{Y}\!X_{2}\,,\,X_{1}\,)$,\,
	\,$\pi_{2} \in \Mor_{\mathfrak{A}}(\,X_{1}\sqcap_{Y}\!X_{2}\,,\,X_{2}\,)$.\,
	\begin{center}
	\begin{tikzcd}
	&& {\color{blue}X_{1}}
		\arrow[ddrr, "f_{1}", blue]
	\\ \\
	{\color{red}X_{1} \sqcap_{Y}\! X_{2}}
		\arrow[uurr, "{\color{red}\pi_{1}}", red]
		\arrow[ddrr, "{\color{red}\pi_{2}}", red, swap]
	&&&&
	{\color{blue}Y}
	\\ \\
	&& {\color{blue}X_{2}}
		\arrow[uurr, "f_{2}", swap, blue]
	\end{tikzcd}
	\end{center}
	Then, the following statements are true:
	\begin{itemize}
	\item
		If \,$f_{2}$\, is an {\color{red}epi}morphism, then \,$\pi_{1}$\, is an {\color{red}epi}morphism.
	\item
	If \,$f_{1}$\, is an {\color{red}epi}morphism, then \,$\pi_{2}$\, is an {\color{red}epi}morphism.
	\end{itemize}
\item
	Dually, let
	\,$\left(\,X_{1}\overset{{\color{white}.}}{\sqcup_{Y}}\!X_{2} \,,\, \iota_{1} \,,\, \iota_{2}\,\right)$\,
	be a fiber coproduct of two morphisms
	\,$f_{1} \in \Mor_{\mathfrak{A}}(Y,X_{1})$\,
	and
	\,$f_{2} \in \Mor_{\mathfrak{A}}(Y,X_{2})$,\,
	where
	\,$X_{1},\, X_{2},\, Y,\, X_{1}\sqcup_{Y}\!X_{2} \,\in\, \Obj(\mathfrak{C})$,\,
	\,$\iota_{1} \in \Mor_{\mathfrak{A}}(\,X_{1}\,,\,X_{1}\sqcup_{Y}\!X_{2}\,)$,\,
	\,$\iota_{2} \in \Mor_{\mathfrak{A}}(\,X_{2}\,,\,X_{1}\sqcup_{Y}\!X_{2}\,)$.\,
	\begin{center}
	\begin{tikzcd}
	&& {\color{blue}X_{1}}
		\arrow[ddll, "{\color{red}\iota_{1}}", red, swap]
	\\ \\
	{\color{red}X_{1} \sqcup_{Y}\! X_{2}}
	&&&&
	{\color{blue}Y}
		\arrow[uull, "f_{1}", blue, swap]
		\arrow[ddll, "f_{2}", blue]
	\\ \\
	&& {\color{blue}X_{2}}
		\arrow[uull, "{\color{red}\iota_{2}}", red]
	\end{tikzcd}
	\end{center}
	Then, the following statements are true:
	\begin{itemize}
	\item
		If \,$f_{2}$\, is a {\color{red}mono}morphism, then \,$\iota_{1}$\, is a {\color{red}mono}morphism.
	\item
		If \,$f_{1}$\, is a {\color{red}mono}morphism, then \,$\iota_{2}$\, is a {\color{red}mono}morphism.
	\end{itemize}
\end{enumerate}
\end{proposition}
\proof
\begin{enumerate}
\item
	We prove only the first implication, i.e.,
	that \,$f_{2}$\, is an epimorphism implies that \,$\pi_{1}$\, is an epimorphism.
	The other implication follows by symmetry:
	\,$(\,f_{2},\pi_{1}\,) \,\longleftrightarrow\, (\,f_{1},\pi_{2}\,)$.\,

	\vskip 0.3cm
	\noindent
	So, we now assume that \,$f_{2}$\, is an epimorphism.
	Consider the defining diagram of \,$X_{1} \sqcap_{Y}\! X_{2}$\,:
	\begin{center}
	\begin{tikzcd}
	&&&& {\color{blue}X_{1}}
		\arrow[ddrr, "f_{1}", blue]
		\arrow[ddl, bend left = 10, "\iota_{1}", hook, gray]
	\\ \\
	0
		\arrow[rr]
		% \arrow[uurrrr, bend left =  20, "h_{1}"]
		% \arrow[ddrrrr, bend left = -20, "h_{2}", swap]
		% \arrow[rrr, bend left = -20, "\psi\,:=\,\iota_{1}\circ h_{1} + \iota_{2}\circ h_{2}", swap]
		% \arrow[rrr, bend left = -20, "\psi", swap]
	&&
	%{\color{red}K}
	{\color{red}X_{1} \sqcap_{Y}\! X_{2}}
		\arrow[r, "\kappa", hook, gray]
		\arrow[uurr, "{\color{red}\pi_{1}}", bend left =  10, red]
		\arrow[ddrr, "{\color{red}\pi_{2}}", bend left = -10, red, swap]
	&
	{\color{gray}X_{1} \oplus X_{2}}
		\arrow[rrr, "\varphi\,:=\,f_{1} \circ\, p_{1} \,-\, f_{2} \circ\, p_{2}", gray]
		\arrow[uur, "p_{1}", bend left =  10, gray]
		\arrow[ddr, "p_{2}", bend left = -10, gray, swap]
	&&&
	{\color{blue}Y}
	\\ \\
	&&&& {\color{blue}X_{2}}
		\arrow[uurr, "f_{2}", swap, blue]
		\arrow[uul, bend left = -10, "\iota_{2}", hook, gray, swap]
	\end{tikzcd}
	\end{center}

	\vskip 0.3cm
	\noindent
	\textbf{Claim 1:}\quad
	For any morphism
	\,$\psi \in \Mor_{\mathfrak{A}}(Y,Z)$,\,
	we have:
	\begin{equation*}
	\psi \circ (\,f_{1} \circ p_{1} - f_{2} \circ p_{2}\,) \,=\, 0
	\quad\Longrightarrow\quad
	\psi \,=\, 0
	\end{equation*}
	\vskip 0.01cm
	\noindent
	Proof of Claim 1:\;\;
	\begin{eqnarray*}
	\psi \circ (\,f_{1} \circ p_{1} - f_{2} \circ p_{2}\,) \,=\, 0
	& \Longrightarrow &
		\psi \circ (\,f_{1} \circ p_{1} - f_{2} \circ p_{2}\,)\; {\color{red}\circ\;\iota_{2}} \,=\, 0
	\\
	& \Longrightarrow &
		(\,\psi \circ f_{1}\,) \circ (\,p_{1} \circ \iota_{2}\,)
		-
		(\,\psi \circ f_{2}\,) \circ (\,p_{2} \circ \iota_{2}\,)
		\,=\, 0
	\\
	& \Longrightarrow &
		(\,\psi \circ f_{1}\,) \circ (\,0_{X_{2},X_{1}}\,)
		-
		(\,\psi \circ f_{2}\,) \circ (\,1_{X_{2}}\,)
		\,=\, 0
	\\
	& \Longrightarrow &
		\psi \circ f_{2} \,=\, 0
	\end{eqnarray*}
	The epimorphicity (``cancellable on the right'') hypothesis on \,$f_{2}$\,
	now implies that \,$\psi = 0$.\,
	This completes the proof of Claim 1.

	\vskip 0.3cm
	\noindent
	\textbf{Claim 2:}\quad
	\,$\varphi \,:=\, f_{1} \circ p_{1} - f_{2} \circ p_{2}$\,
	is a cokernel of \,$\kappa$.\,
	\vskip 0.01cm
	\noindent
	Proof of Claim 2:\;\;
	By construction/definition, \,$\kappa$\, is a kernel of 
	\,$\varphi \,:=\, f_{1} \circ p_{1} - f_{2} \circ p_{2}$.\,
	Next, note that Claim 1 and Lemma \ref{AlternativeCharacterizationOfEpimorphismsInAbelianCategories}
	together imply the epimorphicity of
	\,$\varphi \,:=\, f_{1} \circ p_{1} - f_{2} \circ p_{2}$.\,
	Hence, \,$\varphi$\, is a cokernel, by definition of abelian categories.
	Lastly, by Lemma \ref{AKernelsTheKernelOfItsOwnCokernel}(ii),
	\,$\varphi \,:=\, f_{1} \circ p_{1} - f_{2} \circ p_{2}$\,
	is a cokernel of its kernel \,$\kappa$.\,
	This completes the proof of Claim 2.

	\vskip 0.3cm
	\noindent
	\textbf{Claim 3:}\quad
	For any morphism
	\,$\sigma \in \Mor_{\mathfrak{A}}(X_{1},Z)$,\,
	we have:
	\begin{equation*}
	\sigma \,\circ\, \pi_{1} \,=\, 0
	\quad\Longrightarrow\quad
	\sigma \,=\, 0
	\end{equation*}
	\vskip 0.01cm
	\noindent
	Proof of Claim 3:\;\;
	Suppose \,$\sigma \,\circ\, \pi_{1} \,=\, 0$.\,
	Since \,$\pi_{1} = p_{1} \circ \kappa$,\,
	it follow that
	\,$(\,\sigma \,\circ\, p_{1}\,) \,\circ\, \kappa
	\,=\, \sigma \,\circ\, (\,p_{1} \,\circ\, \kappa\,)
	\,=\, \sigma \,\circ\, \pi_{1}
	\,=\, 0$.\,
	Thus, by the universal property of
	\,$\varphi := f_{1} \circ p_{1} - f_{2} \circ p_{2}$\,
	as a cokernel of \,$\kappa$ (see Claim 2),\,
	there exists a unique
	\,$\sigma^{\prime} \in \Mor_{\mathfrak{A}}(\,Y,Z\,)$\,
	such that
	\,$\sigma \circ p_{1} = \sigma^{\prime} \circ \varphi$\,;\,
	see the following diagram:
	\begin{center}
	\begin{tikzcd}
	& & & {\color{red}Y}
		\arrow[dd, thick, dashed, "\;\exists !\,\sigma^{\prime}", blue]
	\\
	X_{1} \sqcap_{Y}\! X_{2} 
		\arrow[rr, "{\color{white}.}\kappa"]
		\arrow[rrru, bend left = 15, "0", gray]
		\arrow[rrrd, bend right = 15, swap, "0", blue]
		& &
		{\color{red}X_{1} \oplus X_{2}}
		\arrow[rd, blue, thick, "\sigma\,\circ\,p_{1}", swap]
		\arrow[ur, "\varphi", red]
		&
	\\
	& & & {\color{blue}Z}
	\end{tikzcd}
	\end{center}
	Now, recall \,$p_{1} \circ \iota_{2} = 0$,\, hence:
	\begin{eqnarray*}
	0
	& = &
		\sigma \circ (\,p_{1} \circ \iota_{2}\,)
	\;\; = \;\;
		(\,\sigma \circ p_{1}\,) \circ \iota_{2}
	\;\; = \;\;
		(\,\sigma^{\prime} \circ \varphi\,) \circ \iota_{2}
	\;\; = \;\;
		\sigma^{\prime} \circ (\,f_{1} \circ p_{1} - f_{2} \circ p_{2}\,) \circ \iota_{2}
	\\
	& = &
		\sigma^{\prime} \circ (\,f_{1} \circ p_{1} \circ \iota_{2} - f_{2} \circ p_{2} \circ \iota_{2}\,) 
	\;\; = \;\;
		 \sigma^{\prime} \circ f_{1} \circ 0_{X_{2},X_{1}}
		 \, - \,
		 \sigma^{\prime} \circ f_{2} \circ 1_{X_{2}}
	\\
	& = &
		-\, \sigma^{\prime} \circ f_{2}\,,
	\end{eqnarray*}
	which is equivalent to
	\,$\sigma^{\prime} \circ f_{2} \,=\, 0$.\,
	Epimorphicity of \,$f_{2}$\, now implies \,$\sigma^{\prime} = 0$.\,
	Now, observe:
	\begin{eqnarray*}
	\sigma
	& = &
		\sigma \circ 1_{X_{1}}
	\;\; = \;\;
		\sigma \circ (\,p_{1} \circ \iota_{1}\,)
	\;\; = \;\;
		(\,\sigma \circ p_{1}\,) \circ \iota_{1}
	\;\; = \;\;
		(\,\sigma^{\prime} \circ \varphi\,) \circ \iota_{1}
	\;\; = \;\;
		\sigma^{\prime} \circ (\,\varphi \circ \iota_{1}\,)
	\\
	& = &
		0 \circ (\,\varphi \circ \iota_{1}\,)\,,
		\quad
		\textnormal{since we have established that \,$\sigma^{\prime} = 0$}
	\\
	& = &
		0
	\end{eqnarray*}
	This completes the proof of Claim 3.

	\vskip 0.3cm
	\noindent
	Lastly, note that Claim 3 and Lemma \ref{AlternativeCharacterizationOfEpimorphismsInAbelianCategories}
	together immediately imply the epimorphicity of \,$\pi_{1}$,\, as required.

\item
	This follows by dualizing the arguments given above for (i).
	\qed
\end{enumerate}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{proposition}[Lemma 8.3.12, p.180, \cite{kashiwara2005categories}]
\label{NecessarySufficientConditionForExactnessInAbelianCategories}
\mbox{}
\vskip 0.1cm
\noindent
Suppose \,$\mathfrak{A}$\, is an abelian category,
\,$f \in \Mor_{\mathfrak{A}}(X^{\prime},X)$, and $g \in \Mor_{\mathfrak{A}}(X,X^{\prime\prime})$\,
such that
\,$g \circ f = 0$.\,
Then, the following are equivalent:
\begin{enumerate}
\item
	$X^{\prime} \overset{f}{\longrightarrow} X \overset{g}{\longrightarrow} X^{\prime\prime}$\,
	is exact.
\item
	For any morphism
	\,$h \in \Mor_{\mathfrak{A}}(A,X)$\, such that \,$g \,\circ\, h \,=\, 0$,\,
	there exist an epimorphism
	\,$f^{\prime} \,\in\, \Mor_{\mathfrak{A}}(A^{\prime},A)$\,
	and a morphism
	\,$h^{\prime} \,\in\, \Mor_{\mathfrak{A}}(A^{\prime},X^{\prime})$\,
	such that
	\,$f \,\circ\, h^{\prime} \,=\, h \,\circ\, f^{\prime}$.\,
	\begin{center}
	\begin{tikzcd}
	{\color{red}A^{\prime}}
		\arrow[rr, red, thick, dashed, two heads, "f^{\prime}"]
		\arrow[dd, red, thick, dashed, "h^{\prime}", swap]
	&&
	{\color{blue}A}
		\arrow[dd, blue, thick, "h"]
		\arrow[ddrr, gray, bend left = 30, "0"]
	&&
	\\ \\
	X^{\prime}
		\arrow[rr, thick, "f", swap]
	&&
	X
		\arrow[rr, thick, "g", swap]
	&&
	X^{\prime\prime}
	\end{tikzcd}
	\end{center}
\end{enumerate}
\end{proposition}
\proof
\vskip 0.1cm
\noindent
\underline{{\color{white}.}(i)\;\,$\Longrightarrow$\;(ii){\color{white}.}}
\vskip 0.1cm
\noindent
Assume \,(i)\, is true, i.e.,
\,$X^{\prime} \overset{f}{\longrightarrow} X \overset{g}{\longrightarrow} X^{\prime\prime}$\,
is exact.
Recall from Definition \ref{defnExactness} that
the exactness hypothesis (i) is equivalent to the fact that the unique morphism
\,$\theta_{0} \,\in\, \Mor_{\mathfrak{A}}(I_{f},K_{g})$\,
in the commutative diagram below is an isomorphism:
\begin{center}
\begin{tikzcd}
&& && {\color{blue}Q_{f}}
	\arrow[dd, dashed, "\psi"]
\\ \\
X^{\prime}
	\arrow[rr, thick, "f"]
	\arrow[dd, red, thick, dashed, two heads, "\widetilde{f}\;", swap]
	\arrow[rrrr, bend left = 27, "0", gray]
&&
X
	\arrow[rr, thick, "g"]
	\arrow[uurr, blue, two heads, "\pi_{f}"]
&&
X^{\prime\prime}
\\ \\
{\color{blue}I_{f}}
	\arrow[uurr, blue, hook, "\kappa(\pi_{f})"]
	\arrow[rr, red, thick, dashed, hook, "\exists !\,\theta_{0}", swap]
&&
K_{g}
	\arrow[uu, hook, swap, "\kappa_{g}"]
	%\arrow[rr, thick, two heads, swap, "\pi_{\theta}", red]
&&
%Q_{\theta}
\end{tikzcd}
\end{center}
Next, note that, since \,$g \circ h = 0$,\,
by the universal property of \,$\kappa_{g}$\, as a kernel of \,$g$,\,
there exists a unique morphism
\,$\theta_{h} \in \Mor_{\mathfrak{A}}(A,K_{g})$\,
such that
\,$h = \kappa_{g} \circ \theta_{h}$.\,
So, choosing
\,$A^{\prime} \,:=\, X^{\prime}\,\sqcap_{K_{g}}\!A$\,
(hence, $f^{\prime} := \pi_{A}$ and $h^{\prime} := \pi_{X^{\prime}}$),
we have the following diagram:
\begin{center}
\begin{tikzcd}
{\color{red}X^{\prime}\,\sqcap_{K_{g}}\!A}
	\arrow[rr, red, thick, dashed, "\pi_{A}"]
	\arrow[dd, red, thick, dashed, "\pi_{X^{\prime}}", swap]
&&
{\color{blue}A}
	\arrow[ddrr, blue, thick, "h"]
	\arrow[rrrrdd, gray, bend left = 20, "0"]
	\arrow[dd, red, thick, dashed, "\theta_{h}", swap]
&&
% {\color{blue}A}
% 	\arrow[dd, blue, thick, "h"]
% 	\arrow[ddrr, blue, "0"]
% 	\arrow[ddll, red, thick, dashed, "\theta_{h}"]
&&
\\ \\
X^{\prime}
	\arrow[rrrr, thick, bend right = 30, "f", swap]
	\arrow[rr, gray, thick, two heads, "\widetilde{f}", swap]
&&
{\color{gray}I_{f} \cong K_{g}}
	\arrow[rr, gray, thick, hook, "\kappa_{g}", swap]
&&
X
	\arrow[rr, thick, "g", swap]
&&
X^{\prime\prime}
\end{tikzcd}
\end{center}
Lastly, note that, by Proposition \ref{FiberProductsPreserveEpimorphismsInAbelianCategories}(i),
the morphism \,$\pi_{A}$\, is an epimorphism.
This completes the proof of \;(i)\,$\Longrightarrow$\,(ii).\;


\vskip 0.5cm
\noindent
\underline{{\color{white}.}(ii)\;\,$\Longrightarrow$\;(i){\color{white}.}}
\vskip 0.1cm
\noindent
Conversely, assume \,(ii)\, is true.
We need to establish that
\begin{tikzcd}
X^{\prime}
	\arrow[r, "f"]
&
X
	\arrow[r, "g"]
&
X^{\prime\prime}
\end{tikzcd}
is exact.
\vskip 0.2cm
\noindent
Since \,$g \,\circ\, \kappa_{g} \,=\, 0$,\,
the hypothesis \,(ii)\, implies the existence of 
a morphism \,$\alpha \,\in\, \Mor_{\mathfrak{A}}(A^{\prime},X^{\prime})$\,
and
an epimorphism \,$\varepsilon \,\in\, \Mor_{\mathfrak{A}}(A^{\prime},K_{g})$\,
such that
\,$f \,\circ\, \alpha \,=\, \kappa_{g} \,\circ\, \varepsilon$.\,
By Proposition \ref{propositionHomology},
the morphism \,$f$\, factors as
\,$f \,=\, \kappa_{g} \,\circ\, \theta \,\circ\, \widetilde{f}$,\,
where
\,$\theta$\, is a monomorphism and
\,$\widetilde{f}$\, is an epimorphism (since $\widetilde{f}$ is a cokernel).
We thus have the following diagram:
\begin{center}
\begin{tikzcd}
{\color{red}A^{\prime}}
	\arrow[rrrrr, red, thick, dashed, two heads, "\varepsilon"]
	\arrow[dd,    red, thick, dashed, "\alpha", swap]
&&&&&
{\color{blue}A = K_{g}}
	\arrow[dd, blue, thick, hook, "\kappa_{g}"]
	\arrow[ddrr, gray, bend left = 20, "0"]
	\arrow[ddll, gray, "1_{K_{g}}", swap]
\\ \\
X^{\prime}
	\arrow[rr, thick, two heads, "\widetilde{f}\;\;\;"]
	\arrow[rrrrr, thick, bend right = 30, "f", swap	]
&&
I_{f}
	\arrow[r, thick, hook, "\theta"]
&
K_{g}
	\arrow[rr, thick, hook, "\kappa_{g}"]
&&
X
	\arrow[rr, thick, "g"]
&&
X^{\prime\prime}
\end{tikzcd}
\end{center}
We first remark that we have
\,$\theta \,\circ\, \widetilde{f} \,\circ\, \alpha \,=\, \varepsilon$.\,
Indeed, simply oberve:
\begin{eqnarray*}
f \,\circ\, \alpha \; = \; \kappa_{g} \,\circ\, \varepsilon
& \Longrightarrow &
	\kappa_{g} \,\circ\, \theta \,\circ\, \widetilde{f} \,\circ\, \alpha
	\; = \;
	\kappa_{g} \,\circ\, \varepsilon
\\
& \Longrightarrow &
	\theta \,\circ\, \widetilde{f} \,\circ\, \alpha
	\; = \;
	\varepsilon\,,
	\quad\textnormal{since \,$\kappa_{g}$\, is a monomorphism (being a kernel)}
\end{eqnarray*}
Now, we are seeking to establish that
\begin{tikzcd}
X^{\prime}
	\arrow[r, "f"]
&
X
	\arrow[r, "g"]
&
X^{\prime\prime}
\end{tikzcd}
is exact; recall:
\begin{eqnarray*}
\begin{tikzcd}
X^{\prime}
	\arrow[r, "f"]
&
X
	\arrow[r, "g"]
&
X^{\prime\prime}
\end{tikzcd}
\;\;\textnormal{is exact}
& \Longleftrightarrow &
	\textnormal{$\theta$\, is an isomorphism}\,,
	\;\,\textnormal{by Definition \ref{defnExactness}}
\\
& \Longleftrightarrow &
	\textnormal{$\theta$\, is both a monomorphism and an epimorphism}\,,
	\;\,\textnormal{by Proposition \ref{AbelianImpliesIsomorphismIFFMonoEpi}}
\end{eqnarray*}
By Proposition \ref{propositionHomology}, we already know that \,$\theta$\, is a monomorphism.
So, it remains only to establish that \,$\theta$\, is an epimorphism.
To this end, observe that:
\begin{eqnarray*}
h_{1} \,\circ\, \theta \;=\; h_{2} \,\circ\, \theta
& \Longrightarrow &
	h_{1} \,\circ\, \theta \,\circ\, \widetilde{f} \,\circ\, \alpha
	\; = \;
	h_{2} \,\circ\, \theta \,\circ\, \widetilde{f} \,\circ\, \alpha
\\
& \Longrightarrow &
	h_{1} \,\circ\, \varepsilon
	\; = \;
	h_{2} \,\circ\, \varepsilon\,,
	\quad
	\textnormal{since
		\,$\theta \,\circ\, \widetilde{f} \,\circ\, \alpha \,=\, \varepsilon$\,
		(as established above)
		}
\\
& \Longrightarrow &
	h_{1} \; = \; h_{2}\,,
	\quad
	\textnormal{since \,$\varepsilon$\, is an epimorphism, by hypothesis (ii)}
\end{eqnarray*}
This proves that \,$\theta$\, is indeed an epimorphism;
hence, \,$\theta$\, is an isomorphism;
hence,
\begin{tikzcd}
X^{\prime}
	\arrow[r, "f"]
&
X
	\arrow[r, "g"]
&
X^{\prime\prime}
\end{tikzcd}
is indeed exact.
This completes the proof of \;(ii)\,$\Longrightarrow$\,(i),\;
as well as the proof of the Proposition. \qed
% \begin{center}
% \begin{tikzcd}
% && && && {\color{blue}Q_{f}}
% 	\arrow[dd, blue, thick, dashed, "\psi"]
% \\ \\
% &&
% X^{\prime}
% 	\arrow[rr, thick, "f"]
% 	\arrow[dd, red, thick, dashed, two heads, "\widetilde{f}\;", swap]
% 	\arrow[rrrr, bend left = 27, "0", gray]
% &&
% X
% 	\arrow[rr, thick, "g"]
% 	\arrow[uurr, blue, thick, two heads, "\pi_{f}"]
% &&
% X^{\prime\prime}
% \\ \\
% &&
% {\color{blue}I_{f}}
% 	\arrow[uurr, blue, thick, hook, "\kappa(\pi_{f})"]
% 	\arrow[rr, red, thick, dashed, hook, "\exists !\,\theta", swap]
% &&
% K_{g}
% 	\arrow[uu, hook, "\kappa_{g}", swap]
% 	%\arrow[rr, thick, two heads, swap, "\pi_{\theta}", red]
% &&
% %Q_{\theta}
% \\ 
% &
% A^{\prime}
% 	\arrow[urrr, thick, bend right = 20, two heads, "f^{\prime}", swap]
% 	\arrow[uuur, thick, bend left  = 20, "\kappa_{g}^{\prime}"]
% 	\arrow[ur, red, thick, dashed, "\rho"]
% % \\
% % A^{\prime}
% % 	\arrow[uurrrr, thick, bend right = 20, two heads, "f^{\prime}", swap]
% % 	\arrow[uuuurr, thick, bend left  = 20, "\kappa_{g}^{\prime}"]
% \end{tikzcd}
% \end{center}
% \begin{center}
% \begin{tikzcd}
% A^{\prime}
% 	\arrow[rrr, thick, dashed, two heads, "\varepsilon"]
% 	\arrow[dd, thick, dashed, "\alpha", swap]
% &&&
% A = K_{g}
% 	\arrow[dd, thick, "\;1_{K_{g}}"]
% 	\arrow[ddrr, thick, "\kappa_{g}"]
% 	\arrow[ddrrrr, gray, bend left = 20, "0"]
% \\ \\
% X^{\prime}
% 	\arrow[rr, thick, two heads, "\widetilde{f}\;\;\;"]
% 	\arrow[rrrrr, thick, bend right = 30, "f", swap	]
% &&
% I_{f}
% 	\arrow[r, thick, hook, "\theta"]
% &
% K_{g}
% 	\arrow[rr, thick, hook, "\kappa_{g}"]
% &&
% X
% 	\arrow[rr, thick, "g"]
% &&
% X^{\prime\prime}
% \end{tikzcd}
% \end{center}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\vskip 0.5cm
\begin{lemma}[Lemma 8.3.11(a)(i), p.180, \cite{kashiwara2005categories}]
\label{FiberProductKernalMonomorphism}
\mbox{}
\vskip 0.1cm
\noindent
Suppose:
\begin{itemize}
\item
	$\mathfrak{A}$\, be an {\color{red}abelian} category,
\item
	$\left(\,X_{1}\overset{{\color{white}.}}{\sqcap_{Y}}\!X_{2} \,,\, \pi_{1} \,,\, \pi_{2}\,\right)$\,
	is a fiber product of two morphisms
	\,$f_{1} \in \Mor_{\mathfrak{A}}(X_{1},Y)$\,
	and
	\,$f_{2} \in \Mor_{\mathfrak{A}}(X_{2},Y)$,\,
	where
	\,$X_{1},\, X_{2},\, Y,\, X_{1}\sqcap_{Y}\!X_{2} \,\in\, \Obj(\mathfrak{C})$,\,
	\,$\pi_{1} \in \Mor_{\mathfrak{A}}(\,X_{1}\sqcap_{Y}\!X_{2}\,,\,X_{1}\,)$,\,
	\,$\pi_{2} \in \Mor_{\mathfrak{A}}(\,X_{1}\sqcap_{Y}\!X_{2}\,,\,X_{2}\,)$,\,
\item
	$
	\kappa(\pi_{1})
	\,\in\,
	\Mor_{\mathfrak{A}}\!\left(\,
		K(\pi_{1})
		\,,\,
		X_{1}\overset{{\color{white}.}}{\sqcap_{Y}}\!X_{2}
		\,\right)
	$\,
	is a kernel of \,$\pi_{1}$,\, and
\item
	$
	\kappa(f_{2})
	\,\in\,
	\Mor_{\mathfrak{A}}\!\left(\,
		\overset{{\color{white}.}}{K(f_{2})}
		\,,\,
		X_{2}
		\,\right)
	$\,
	be a kernel of \,$f_{2}$.\,
\end{itemize}
Then, the following statements are true:
\begin{enumerate}
\item
	There exists a unique
	\;$\alpha \,\in\, \Mor_{\mathfrak{A}}\!\left(\,\overset{{\color{white}.}}{K(\pi_{1})}\,,\,K(f_{2})\,\right)$\;
	such that
	\;$\kappa(f_{2}) \,\circ\, \alpha \,=\, \pi_{2} \,\circ\, \kappa(\pi_{1})$.\;
	\begin{center}
	\begin{tikzcd}
	K(\pi_{1})
		\arrow[rr, thick, hook, "\kappa(\pi_{1})"]
		\arrow[dd, red, thick, dashed, "\alpha", swap]
	&&
	{\color{black}X_{1} \sqcap_{Y}\! X_{2}}
		\arrow[rr, thick, "\pi_{1}"]
		\arrow[dd, thick, "\pi_{2}", swap]
	&&
	X_{1}
		\arrow[dd, thick, "f_{1}"]
	\\ \\
	K(f_{2})
		\arrow[rr, thick, hook, "\kappa(f_{2})", swap]
	&&
	X_{2}
		\arrow[rr, thick, "f_{2}", swap]
	&&
	Y
	\end{tikzcd}
	\end{center}
\item
	If \,$f_{1}$\, is furthermore a monomorphism,
	then \,$\alpha$\, is also a monomorphism.
	\begin{center}
	\begin{tikzcd}
	K(\pi_{1})
		\arrow[rr, thick, hook, "\kappa(\pi_{1})"]
		\arrow[dd, red, thick, hook, dashed, "\alpha", swap]
	&&
	{\color{black}X_{1} \sqcap_{Y}\! X_{2}}
		\arrow[rr, thick, "\pi_{1}"]
		\arrow[dd, thick, "\pi_{2}", swap]
	&&
	X_{1}
		\arrow[dd, blue, hook, thick, "f_{1}"]
	\\ \\
	K(f_{2})
		\arrow[rr, thick, hook, "\kappa(f_{2})", swap]
	&&
	X_{2}
		\arrow[rr, thick, "f_{2}", swap]
	&&
	Y
	\end{tikzcd}
	\end{center}
\end{enumerate}
\end{lemma}
\proof
\begin{enumerate}
\item
	Observe that
	\begin{eqnarray*}
	f_{2} \,\circ \left(\,\pi_{2} \overset{{\color{white}1}}{\circ} \kappa(\pi_{1})\,\right)
	& = &
		\left(\,f_{2} \overset{{\color{white}1}}{\circ} \pi_{2}\,\right) \circ\, \kappa(\pi_{1})
	\;\; = \;\;
		\left(\,f_{1} \overset{{\color{white}1}}{\circ} \pi_{1}\,\right) \circ\, \kappa(\pi_{1})
	\;\; = \;\;
		f_{1} \,\circ \left(\,\pi_{1} \overset{{\color{white}1}}{\circ} \kappa(\pi_{1})\,\right) 
	\;\; = \;\;
		f_{1} \,\circ\, 0
	\;\; = \;\;
		0\,,
	\end{eqnarray*}
	which implies
	\,--\, by the universal property of \,$\kappa(f_{2})$\, as a kernel of \,$f_{2}$ \,--\,
	that there exists a unique
	\;$\alpha \,\in\, \Mor_{\mathfrak{A}}\!\left(\,\overset{{\color{white}.}}{K(\pi_{1})}\,,\,K(f_{2})\,\right)$\;
	such that
	\;$\kappa(f_{2}) \,\circ\, \alpha \,=\, \pi_{2} \,\circ\, \kappa(\pi_{1})$.\;
\item
	Suppose
	\,$g,\, h \,\in\, \Mor_{\mathfrak{A}}(\,A\,,\,K(\pi_{1})\,)$\,
	are two arbitrary morphisms satisfying
	\,$\alpha \,\circ\, g \,=\, \alpha \,\circ\, h$.\,
	We need to show that \,$g \,=\, h$.\,
	To this end, note
	\begin{eqnarray*}
	\alpha \,\circ\, g \,=\, \alpha \,\circ\, h
	& \Longrightarrow &
		\kappa(f_{2}) \,\circ\, \alpha \,\circ\, g \;=\; \kappa(f_{2}) \,\circ\, \alpha \,\circ\, h
	\\
	& \Longrightarrow &
		\pi_{2} \,\circ\, \kappa(\pi_{1}) \,\circ\, g \;=\; \pi_{2} \,\circ\, \kappa(\pi_{1}) \,\circ\, h\,,
		\quad
		\textnormal{by (i)}
	\\
	& \Longrightarrow &
		\kappa(\pi_{1}) \,\circ\, g \;=\; \kappa(\pi_{1}) \,\circ\, h\,,
		\quad
		\textnormal{since \,$\pi_{2}$\, is a monomorphism,
			by Proposition \ref{FiberProductsPreserveMonomorphisms}(i)
			}
	\\
	& \Longrightarrow &
		g \;=\; h\,,
		\quad
		\textnormal{since kernels are monomorphisms,
			by Proposition \ref{KernelsAreMonoCokernelsAreEpi}
			}
	\end{eqnarray*}
	This completes the proof of the monomorphicity of \,$\alpha$.\,
	\qed
\end{enumerate}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%
