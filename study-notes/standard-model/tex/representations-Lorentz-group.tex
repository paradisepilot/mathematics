...
\chapter{Irreducible representations of the Lorentz algebra $\mathfrak{so}^{+}(1,3)$}
...
\vskip 0.5cm
\begin{definition}[$\textnormal{SO}(1,n)$\, and \,$\textnormal{SO}^{\uparrow}(1,n)$]
\mbox{}
\vskip 0.1cm
\noindent
...
\vskip 0.5cm
\begin{proposition}[Parametrization of \,$\mathfrak{so}(1,3)$]
\mbox{}
\vskip 0.1cm
\noindent
$\mathfrak{so}(1,3)$ admits the following parametrization:
\begin{equation*}
\mathfrak{so}{(1,3)}
\; = \;
	\left\{\;
...
\end{equation*}
In particular,
\,$\dim_{\Re}\!\left(\overset{{\color{white}.}}{\textnormal{SO}^{\uparrow}(1,3)}\right)$
\,$=$\,
\,$\dim_{\Re}\!\left(\overset{{\color{white}.}}{\mathfrak{so}(1,3)}\right)$
\,$=$\, $6$.\,
\end{proposition}
\proof
Suppose:
