...
is a principal bundle and
\,$\mathfrak{g}$\, is the Lie algebra of the structure group \,$G$.\,
Let \,$f \in \mathscr{G}(P)$\, be a gauge transformation and
\,$\omega \in \Omega^{1}(P,\mathfrak{g})$\, be a connection $1$-form.
Then,
\begin{enumerate}
\item
	$f^{*}(\omega)$\, is a connection $1$-form on \,$P$,\, and
\item
	...
	\;\; = \;\;
		...
		\sigma_{f}^{*}(\,\mu_{G}\,)\,,
	\end{equation*}
	where \,$\mu_{G} \in \Omega^{1}(G,\mathfrak{g})$\, is the Maurer-Cartan form of \,$G$,\,
	and \,$\sigma_{f} \in \mathscr{C}^{\infty}(P,G)^{G}$\, is defined implicitly by:
	\begin{equation*}
	f(p)
	\;\; = \;\;
		p \cdot \sigma_{f}(p)\,,
...
\item
	...
	% 	\arrow[rrrrrr, bend left = 35, "\sigma_{f}^{*}\,\mu_{G}"]
	% &&&
	...
	% 	\arrow[rrrrrr, bend left = 35, "R_{\sigma_{f}(p)}^{*}\,\omega_{f(p)}"]
	% &&&
	% \mathcal{T}_{f(p)}\,P
	...
	\;\; = \;\;
		...
		\sigma_{f}^{*}(\,\mu_{G}\,)\,,
	\end{equation*}
	which completes the proof of (ii) as well as that of the present Proposition.
	\qed
\end{enumerate}
