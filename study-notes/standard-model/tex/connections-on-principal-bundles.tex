...
\item
	First, note that, for each \,$p \in P$,\, we have:\;
	$f(p) \,=\, p \cdot \sigma_{f}(p) \,=\, R_{\sigma_{f}(p)}\!\left(\,\overset{{\color{white}1}}{p}\,\right)$\,
	\begin{eqnarray*}
	\left(\,F^{\,f^{*}\omega}\,\right)_{\!p}
	& = &
		\left(\,\d(\,\overset{{\color{white}.}}{f^{*}\omega}\,)\,\right)_{p}
		\; + \;
	...
	\\
	& = &
		\left(\,\overset{{\color{white}.}}{f^{*}(\,\d\,\omega\,)}\,\right)_{p}
		\; + \;
	...
	& = &
		f^{*}\!\left(\;
			...
			\,\right)_{\!p}
	\;\; = \;\;
		f^{*}\!\left(\,F^{\omega}_{p}\,\right)
	\;\; = \;\;
		R^{*}_{\sigma_{f}(p)}\!\left(\;F^{\,\omega}_{p}\;\right)
	\\
	& = &
		\Ad_{\sigma_{f}(p)^{-1}} \,\circ\, F^{\,\omega}_{p}\,,
	\end{eqnarray*}
